...
Foram consideradas as observações com no máximo 15 dias de doença, sendo assim, a matrix de correlação é a seguinte:

```{r}
require(geepack)
corr_struc <-  CVTuningCov::AR1(15,0.801	)
corr_struc %>% kable('latex',col.names = paste("Dia",1:15))
```
...
confusionMatrix(ifelse(t_pred<.375,1,0) %>% as.factor,dados_primeira_obs$Desfecho %>% as.factor,positive='1')
```
...
A conclusão que podemos ter com essa tentativa é que o TGO é uma variável muito interessante por si só que poderia ter resultados interessantes caso
fosse necessário a elaboração de um modelo mais simples de ser aplicado.
