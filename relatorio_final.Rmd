...
ggplot(data=dados_plot %>% filter(dia_doenca>0) %>% mutate(Outcome=Desfecho),aes(y=Creatinina,fill=Outcome,x=as.factor(dia_doenca)))+
...
ggplot(dados2 %>% mutate(Outcome = as.factor(Desfecho)),aes(dia_doenca,Creatinina,group=pac,color=Outcome)) + 
